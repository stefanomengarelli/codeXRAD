...
namespace codeXRAD
{
    ...
    public partial class CXDatabase : Component
    {
        ...
        {
            if (Template.Trim().Length > 0) return CX.Combine(CX.OnLibraryPath("Database"), Template, "mdb");
            else return "";
        }
        ...
        public static int Add(string _Alias, string _DatabaseTemplate)
        {
            ...
            if (_DatabaseTemplate.Length < 1) _DatabaseTemplate = CX.ExecutableName;
            if (_Alias.Length > 0)
            {
                db = new CXDatabase();
                db.Alias = _Alias;
        ...
        public static bool Lock()
        {
            if (!Locked())
            {
                return CX.SaveString(CX.Combine(CX.DataPath, CX.ExecutableName, "lck"),
                    CX.ExecutableName + ';' + CX.Str(DateTime.Now) + ';' + CX.Machine() + ";" + CX.User());
            }
            else return false;
        }
        ...
        public static CXDatabaseType TypeFromString(string _DatabaseType)
        {
            ...
            if ((_DatabaseType == "ACCESS") || (_DatabaseType == "0")) return CXDatabaseType.Access;
        ...
        {
            CX.FileDelete(CX.Combine(CX.DataPath, CX.ExecutableName, "lck"));
            return !Locked();
        }
